...
class NNValueFunction(object):
    ...
    def __init__(self, obs_dim, dims_core_hid, dims_head_hid, num_tasks, time_steps_mini_batch = 256, act_func_name = "tan"):
        """
        Args:
    ...
    def fit(self, task, observes, dis_rew, logger):
        ...
        vals_pred = self.predict(observes[start:end], task)
        loss = np.mean(np.square(vals_pred - dis_rew[start:end])) # explained variance after update
        exp_var = 1 - np.var(dis_rew[start:end] - vals_pred) / np.var(dis_rew[start:end])  # diagnose over-fitting of val func
