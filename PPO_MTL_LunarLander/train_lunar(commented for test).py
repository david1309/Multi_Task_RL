...
def main(env_name, num_episodes, gamma, lamda, kl_targ, clipping_range, pol_loss_type, batch_size, init_pol_logvar, animate,\
    ...
    # LunarLander Specific Code
    task_params_dict = {0: 'LunarLanderGoal-v2', 1: 'LunarLanderNotGoal-v2', 2:'LunarLanderFly-v2'}
    task_params = [task_params_dict[param] for param in task_params]

    # ****************  Environment Initialization and Paths  ***************
    task_params_str = 'Goal, NotGoal, Fly'#''.join(e + ', ' for e in task_params)
    ...
    if episode_to_load == None: episode = 0
        ...
        episode = episode_to_load
        num_episodes = episode+1 # only run one episode for the purpose of only saving video of the agent(s)
...
# python train_lunar.py LunarEnv --task_params 0 1 2 --task_name diff_R -dcore 64 32 16 -dhead 128 64 16 --pol_loss_type both --num_episodes 9 --batch_size 3 --save_video False --save_rate 2 --animate True
