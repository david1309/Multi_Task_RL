...
class Policy(object):
    ...
    def __init__(self, obs_dim, act_dim, kl_targ, hid1_mult, policy_logvar, clipping_range=None):
        ...
        """
        self.beta = 1.0  # Beta: gain of D_KL divergance loss term
        self.eta = 50  # Eta: gain of the loss term controling that D_KL does,t exceed KL_targ (hinge loss)
        ...
        self.lr = None
        self.lr_multiplier = 1.0  # dynamically adjust lr based on value of D_KL
    ...
    def _policy_nn(self):
        ...
        """
        # Hidden sizes determined by obs_dim, hid1_mult and act_dim 
        hid1_size = 64#self.obs_dim * self.hid1_mult  
        hid3_size = 64#self.act_dim * 10  # 10 --> MAGIC NUMBER empirically determined
        hid2_size = 64#int(np.sqrt(hid1_size * hid3_size)) # size hid2=g eometric mean hid1 & hid2

        # heuristic to set learning rate based on NN size (tuned on 'Hopper-v1')
        self.lr = 9e-4 / np.sqrt(hid2_size)  # 9e-4 MAGIC NUMBER empirically determined

        # Hidden Layers: 3 hidden layers with tanh activations
        ...
        h3 = tf.layers.dense(h2, hid3_size, tf.tanh,
                              kernel_initializer=tf.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid2_size)), name="h3")

        # Output Layer: Linear layer, activation=None --> linear
        self.means = tf.layers.dense(h3, self.act_dim,
                                     kernel_initializer=tf.random_normal_initializer(
                                         stddev=np.sqrt(1 / hid3_size)), name="means")

        # logvar_speed is used to 'fool' gradient descent into making faster updates
        # to log-variances, since the model is now predicting 'logvar_speed' variances
        # for each action dimension
        logvar_speed = (10 * hid3_size) // 48 # MAGIC NUMBER
        # log_vars is a trainnable variable predicting logvar_speed variances (rows) for
        # each action dimension (columns)
        ...
        print('\nPolicy Network Params -- h1: {}, h2: {}, h3: {}, lr: {:.3g}, logvar_speed: {}'
    ...
    def update(self, observes, actions, advantages, logger):
        ...
        feed_dict = {self.obs_ph: observes, # Used for feedforward pass of Policy Network
                     ...
                     self.beta_ph: self.beta, # Loss gains and learning rate
